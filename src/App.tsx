...
const Navbar = ({ user, onLogout }: { user: User | null; onLogout: () => void }) => (
  <nav className="bg-white border-b border-gray-100 px-6 py-4 flex justify-between items-center sticky top-0 z-50">
    <div className="flex items-center gap-2">
      ...
      <span className="font-bold text-xl tracking-tight text-gray-900">QueueFlow</span>
    </div>
    {user && (
...
const RestaurantRegistration = ({ onRegistered }: { onRegistered: (id: string) => void }) => {
        ...
        {showQR && (
          ...
          >
            <div className="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm flex flex-col items-center text-center">
              ...
              <a href={joinUrl} target="_blank" rel="noreferrer" className="mt-2 text-indigo-600 text-xs hover:underline">
                {joinUrl}
              </a>
            </div>
          </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="space-y-3">
            {queue.filter(e => e.status !== 'completed').map((entry) => (
              <motion.div 
                layout
                key={entry.id}
              ...
              >
                <div className="flex items-center gap-4">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-bold text-lg ${
                    entry.status === 'called' ? 'bg-amber-200 text-amber-800' : 'bg-indigo-100 text-indigo-700'
                  }`}>
                    {entry.tokenNumber}
                  </div>
                  <div>
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
            ...
            <div className="space-y-3">
              {queue.filter(e => e.status === 'completed').slice(-5).reverse().map((entry) => (
                <div key={entry.id} className="flex items-center justify-between text-sm py-2 border-b border-gray-50 last:border-0">
                  ...
                  <span className="text-gray-400">Token #{entry.tokenNumber}</span>
                </div>
              ))}
              {queue.filter(e => e.status === 'completed').length === 0 && (
                <p className="text-xs text-gray-400 italic">No completed entries yet.</p>
              )}
            </div>
          </div>
        </div>
...
const CustomerJoin = () => {
  ...
  const handleJoin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const docRef = await addDoc(collection(db, 'restaurants', restaurantId, 'queue'), {
        customerName: name,
        customerPhone: phone,
      ...
      // Listen for updates to this specific entry
      const unsubscribe = onSnapshot(doc(db, 'restaurants', restaurantId, 'queue', docRef.id), (snapshot) => {
        if (snapshot.exists()) {
          setToken({ id: snapshot.id, ...snapshot.data() } as QueueEntry);
        }
      });
    ...
    return (
      <div className="min-h-screen bg-indigo-600 flex items-center justify-center p-6">
        ...
        >
          ...
          <p className="text-gray-500 mb-8">{restaurant?.name}</p>
          
          <div className="bg-gray-50 rounded-2xl p-8 mb-8">
            <p className="text-sm text-gray-400 uppercase tracking-widest font-bold mb-2">Your Token Number</p>
            <h1 className="text-7xl font-black text-indigo-600">{token.tokenNumber}</h1>
          </div>

          <div className={`p-4 rounded-xl mb-6 flex items-center justify-center gap-3 ${
            token.status === 'called' ? 'bg-amber-100 text-amber-800 animate-pulse' : 'bg-blue-50 text-blue-700'
          }`}>
            {token.status === 'called' ? (
              <>
                <CheckCircle2 className="w-5 h-5" /> 
                <span className="font-bold">It's your turn! Please proceed to counter.</span>
              </>
            ) : token.status === 'completed' ? (
              <>
                <CheckCircle2 className="w-5 h-5" /> 
                <span className="font-bold">Thank you for visiting!</span>
              </>
            ) : (
              <>
                <Clock className="w-5 h-5" /> 
                <span className="font-bold">Please wait for your turn...</span>
              </>
            )}
          </div>
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 mt-1">at {restaurant?.name || 'Restaurant'}</p>
        </div>
          ...
          <div className="max-w-4xl w-full grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            <div className="space-y-6">
              <h1 className="text-5xl font-black text-gray-900 leading-tight">
                Replace physical queues with <span className="text-indigo-600">QueueFlow.</span>
              </h1>
              <p className="text-xl text-gray-500 leading-relaxed">
                The modern way to manage restaurant queues. Customers scan, join, and wait anywhere.
              </p>
              <button 
                onClick={handleLogin}
